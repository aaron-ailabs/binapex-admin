...
export function WithdrawalsTable({ data }: { data: any[] }) {

    const handleApprove = async (id: string) => {
        try {
            const supabase = createClient()
            const { error } = await supabase.rpc("approve_withdrawal", { transaction_id: id })
    ...
    const columns: ColumnDef<any>[] = [
        ...
        },
        {
            accessorKey: "amount",
            header: "Amount",
            cell: ({ row }) => <span className="font-mono font-bold text-red-500">-${row.original.amount}</span>
        },
        {
            accessorKey: "method",
            header: "Method",
            cell: ({ row }) => <span className="text-xs">{row.original.withdrawal_method || "Bank"}</span>
        },
        {
            accessorKey: "status",
            header: "Status",
